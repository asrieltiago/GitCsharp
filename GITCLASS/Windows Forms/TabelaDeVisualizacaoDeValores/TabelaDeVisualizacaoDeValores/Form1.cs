...
namespace TabelaDeVisualizacaoDeValores
{
    public partial class Form1 : Form
    {
        ...
        private void DataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                var collumnId = dataGridView1.Rows[e.RowIndex].Cells[0];
                var collumnValue = dataGridView1.Rows[e.RowIndex].Cells[e.ColumnIndex];
                    ...
                    case 1:
                        {
                                    ...
                                    , MessageBoxIcon.Question) == DialogResult.Yes)
                            {
                                ...
                                x.Id == (int)collumnId.Value).Value = (int)collumnValue.Value;
                            }
                        }
                        break;
                    case 2:
                        {
                            if (MessageBox.Show("Deseja realmente ajustar este valor ?"
                                    ...
                                    , MessageBoxIcon.Question) == DialogResult.Yes)
                            {
                                var dataInformada = DateTime.Parse(collumnValue.Value.ToString());
                                if (dataInformada <= DateTime.Now)
                                    listContracs.FirstOrDefault(x =>
                                    x.Id == (int)collumnId.Value).DatInc = DateTime.Parse(collumnValue.Value.ToString());
                                else
